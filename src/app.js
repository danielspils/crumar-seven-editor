...
(function () {
  ...
  // RESTORED 2026-08-22, having been destroyed the day it shipped. It went in
  // with 7a0db58 and out with 0db93b6 — the refresh-all removal, whose scripted
  // deletion sliced a range that this function happened to sit inside. The
  // sweep was verified gone; nothing verified what else went with it.
  //
  // It was called on every connect and defined nowhere, so connecting threw a
  // ReferenceError out of the 'connected' branch and skipped everything after
  // the if/else chain — connBtn.disabled, backupBtn.hidden and the WRITE GATE
  // BANNER. The prompt itself had been tested on the instrument in both
  ...
  // Named, so the launch-time resume offer can reach sendSetlist — the same
  // path the library's own button takes, rather than a second one.
  const libHandlers = {
